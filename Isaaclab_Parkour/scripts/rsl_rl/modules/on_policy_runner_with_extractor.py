...
def _rsl_obs_dict(policy_obs: torch.Tensor, critic_obs: torch.Tensor, device: str | torch.device):
    """Build the observation dict required by newer rsl_rl storage/PPO APIs."""
    return {
        "policy": policy_obs.to(device),
        "critic": critic_obs.to(device),
    }


def _call_process_env_step_compat(alg, obs_dict, rewards, dones, infos):
    """Call PPO.process_env_step across old and new rsl_rl signatures.

    Newer rsl_rl: process_env_step(obs, rewards, dones, extras)
    Older rsl_rl: process_env_step(rewards, dones, infos)
    """
    rewards = rewards.float()
    try:
        return alg.process_env_step(obs_dict, rewards, dones, infos)
    except TypeError as exc:
        msg = str(exc)
        if "positional" in msg or "required positional argument" in msg or "were given" in msg or "takes" in msg:
            return alg.process_env_step(rewards, dones, infos)
        raise


def _call_compute_returns_compat(alg, obs_dict, privileged_obs):
    """Call PPO.compute_returns across rsl_rl versions.

    Newer rsl_rl expects an observation dict. Older Parkour/rsl_rl forks expect
    the critic/privileged observation tensor directly.
    """
    try:
        return alg.compute_returns(obs_dict)
    except (TypeError, AttributeError, KeyError, IndexError):
        return alg.compute_returns(privileged_obs)
...
class OnPolicyRunnerWithExtractor(OnPolicyRunner):
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        # rsl_rl 3.x calls policy.update_normalization(obs) inside PPO.process_env_step().
        # ActorCriticRMA uses the runner's external EmpiricalNormalization, so this can be a no-op.
        if not hasattr(policy, "update_normalization"):
        ...
        if self.depth_encoder_cfg is None:
            # Current rsl_rl PPO.init_storage signature is:
            # init_storage(training_type, num_envs, num_transitions_per_env, obs, actions_shape)
            # obs must be a dict/TensorDict whose values are sample tensors
            # with shape [num_envs, obs_dim].

            storage_obs = _rsl_obs_dict(
                obs,
                extras["observations"].get(self.privileged_obs_type, obs),
                self.device,
            )

            print("DEBUG init_storage obs:")
            print({k: tuple(v.shape) for k, v in storage_obs.items()})
            print("DEBUG actions_shape:", [self.env.num_actions])
        ...
        for it in range(start_iter, tot_iter):
            ...
            with torch.inference_mode():
                for _ in range(self.num_steps_per_env):
                    ...
                    # process the step
                    step_obs = _rsl_obs_dict(obs, privileged_obs, self.device)
                    _call_process_env_step_compat(self.alg, step_obs, rewards, dones, infos)
                ...
                # compute returns
                if self.training_type == "rl":
                    returns_obs = _rsl_obs_dict(obs, privileged_obs, self.device)
                    _call_compute_returns_compat(self.alg, returns_obs, privileged_obs)
    ...
    def learn_vision(self, num_learning_iterations, init_at_random_ep_len=False):
        ...
        obs, extras = self.env.get_observations()
        additional_obs = {}
        additional_obs["delta_yaw_ok"] = extras['observations']['delta_yaw_ok'].to(self.device).squeeze(-1).bool()
        ...
        for it in range(start_iter, tot_iter):
            ...
            for _ in range(self.depth_encoder_cfg['num_steps_per_env']):
                ...
                if it < num_pretrain_iter:
                    ...
                    # Move to device
                    obs, dones = (obs.to(self.device), dones.to(self.device))
                additional_obs['delta_yaw_ok'] = infos["observations"]['delta_yaw_ok'].to(self.device).squeeze(-1).bool()
