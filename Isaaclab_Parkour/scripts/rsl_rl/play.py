...
def main():
    ...
    timestep = 0
    debug_interval = int(os.environ.get("PARKOUR_DEBUG_INTERVAL", "0"))
    max_steps = int(os.environ.get("PARKOUR_MAX_STEPS", "0"))
    ...
    while simulation_app.is_running():
        ...
        if (
            debug_interval > 0
            and agent_cfg.algorithm.class_name == "DistillationWithExtractor"
            and timestep % debug_interval == 0
        ):
            unwrapped = env.unwrapped
            robot = unwrapped.scene["robot"]
            parkour_event = unwrapped.parkour_manager.get_term("base_parkour")
            root_pos = robot.data.root_pos_w[0]
            root_vel = robot.data.root_lin_vel_b[0]
            print(
                "[parkour-debug] "
                f"step={timestep} pos={root_pos.detach().cpu().tolist()} "
                f"vel_b={root_vel.detach().cpu().tolist()} "
                f"goal_idx={int(parkour_event.cur_goal_idx[0])} "
                f"goal={parkour_event.cur_goals[0].detach().cpu().tolist()} "
                f"next_goal={parkour_event.next_goals[0].detach().cpu().tolist()} "
                f"yaw_obs={obs[0, 6:8].detach().cpu().tolist()} "
                f"scan=({float(obs[0, num_prop:num_prop + num_scan].min()):.3f},"
                f"{float(obs[0, num_prop:num_prop + num_scan].max()):.3f}) "
                f"depth=({float(depth_camera[0].min()):.3f},{float(depth_camera[0].max()):.3f}) "
                f"latent=({float(depth_latent[0].min()):.3f},{float(depth_latent[0].max()):.3f}) "
                f"action=({float(actions[0].min()):.3f},{float(actions[0].max()):.3f})",
                flush=True,
            )
