...
class ParkourManagerBasedEnv(ManagerBasedEnv):
    ...
    def load_managers(self):
        ...
        # -- observation manager
        self._drop_empty_observation_groups()
    ...
    def _drop_empty_observation_groups(self):
        """Remove stale observation groups that contain no observation terms."""
        if self.cfg.observations is None:
            return
        for group_name, group_cfg in list(vars(self.cfg.observations).items()):
            ...
            term_names = [
                ...
                if isinstance(term_cfg, ObservationTermCfg)
            ]
            if not term_names:
                print(
                    f"[INFO] Dropping empty observation group '{group_name}' "
                    f"({type(group_cfg).__name__}) before ObservationManager init."
                )
                setattr(self.cfg.observations, group_name, None)
